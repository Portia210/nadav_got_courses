...
def save_data_to_csv(state: dict):
    entries_list = state["entries_list"]
    if not _has_data(entries_list): return
    file_name = input("Enter the file name (without .csv extention): ")+".csv"
    df = pd.DataFrame(entries_list)
    # df.columns = ['id', 'name', 'age']
    if os.path.exists("conf.json"):
        print("found conf file")
...
def run_menu():
    ...
    ("Save date to csv", lambda x: save_data_to_csv(x)),
